...
def layout_area(agent: SequencePairSaAgentV1) -> int:
...
def run_test(render: bool = True):
    ...
    for tick in range(1, ticks_to_simulate + 1):
        ...
        if render and tick % 2 == 0:
            status = f"SP-SA V1 Area {layout_area(agent)} | Yield {total_yield:.1f} | Tick {tick}"
            render_system_b_blueprint(env, tick=tick, status_text=status)
            time.sleep(0.05)

    print(f"Sequence Pair + SA V1 simulation finished, collected target output: {total_yield:.1f}")
    if total_yield <= 0:
        raise AssertionError("Sequence Pair + SA V1 produced no MID_CAP_BATTERY output.")
