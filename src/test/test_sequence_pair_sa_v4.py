...
def layout_area(agent: SequencePairSaAgentV4) -> int:
...
def run_test(render: bool = True):
    ...
    agent.optimize(env)
    print(f"=== Sequence Pair + SA V4 layout area: {layout_area(agent)} cells ===")
    print(f"=== Routed belts/crossers: {len(env.transports)} | failed routes: {len(agent.failed_routes)} ===")
    if agent.failed_routes:
        raise AssertionError(f"Sequence Pair + SA V4 has unresolved routes: {len(agent.failed_routes)}")

    for building in env.buildings:
        env.update_connections(building)

    total_yield = 0.0
    ticks_to_simulate = 220
            ...
            for ox, oy in out_ports:
                ...
                if cell and type(cell).__name__ == "SystemBBelt":
                    ...
                    if item:
                        ...
                        if item_mat == MaterialType.HIGH_CAP_BATTERY:
                            total_yield += item_amt
                            cell.current_item = None

        if render and tick % 2 == 0:
            status = f"SP-SA V4 Area {layout_area(agent)} | Yield {total_yield:.1f} | Tick {tick}"
            render_system_b_blueprint(env, tick=tick, status_text=status)
            time.sleep(0.05)

    print(f"Sequence Pair + SA V4 simulation finished, collected target output: {total_yield:.1f}")
    if total_yield <= 0:
        raise AssertionError("Sequence Pair + SA V4 produced no HIGH_CAP_BATTERY output.")
