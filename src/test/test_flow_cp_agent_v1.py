...
def layout_area(agent: FlowCpAgentV1) -> int:
...
def run_test(render: bool = True):
    ...
    agent.optimize(env)
    print(f"=== Flow + CP V1 layout area: {layout_area(agent)} cells ===")
    print(f"=== Routed belts/crossers: {len(env.transports)} | failed routes: {len(agent.failed_routes)} ===")
    if agent.failed_routes:
        raise AssertionError(f"Flow + CP V1 has unresolved routes: {len(agent.failed_routes)}")
    ...
    for tick in range(1, ticks_to_simulate + 1):
        ...
        if render and tick % 2 == 0:
            status = f"Flow-CP V1 Area {layout_area(agent)} | Yield {total_yield:.1f} | Tick {tick}"
            render_system_b_blueprint(env, tick=tick, status_text=status)
            time.sleep(0.05)

    print(f"Flow + CP V1 simulation finished, collected target output: {total_yield:.1f}")
    if total_yield <= 0:
        raise AssertionError("Flow + CP V1 produced no MID_CAP_BATTERY output.")
