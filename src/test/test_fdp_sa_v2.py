...
def layout_area(agent: FdpSaAgentV2) -> int:
...
def run_test(render: bool = False):
    random.seed(16)
    env = GridMap(45, 45)
    ...
    agent.optimize(env)
    print(f"=== FDP+SA V2 compact layout area: {layout_area(agent)} cells ===")
    ...
    for tick in range(1, ticks_to_simulate + 1):
        ...
        if render and tick % 2 == 0:
            status = f"V2 Area {layout_area(agent)} | Yield {total_yield:.1f} | Tick {tick}"
            render_system_b_blueprint(env, tick=tick, status_text=status)
            time.sleep(0.05)

    print(f"V2 simulation finished, collected target output: {total_yield:.1f}")


if __name__ == "__main__":
    render_enabled = "--no-render" not in sys.argv
    run_test(render=render_enabled)
